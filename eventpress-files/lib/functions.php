...
if( ! function_exists( 'dump' ) ){
     ...
     * @since 1.0
     *
     * @param variable      $var the variable you want to dump
...
if( ! function_exists( 'hex2rgb' ) ){
    /**
     * Convert hexa-decimal values to RGB values
     *
     * @since 1.0
     *
     * @param string $hex   hexa-decimal values
     *
...
if( ! function_exists( 'get_user_list' ) ){
    /**
     * Get some attributes of all users
     *
     * @since 1.0
     *
     * @param   array   $array  array of fields that needs to be fetched
     *
     * @link https://codex.wordpress.org/Function_Reference/get_users
     *
     * @return  OBJECT          WP_Users object
...
if( ! function_exists( 'get_user_role' ) ){
    /**
     * Get roles of an user or logged in users
     *
     * @since 1.0
     *
     * @param   int|bool    $user_id    ID of an user or false
     * @return  ARRAY                   array of roles of the user
     */
    function get_user_role( $user_id = false ) {
...
if( ! function_exists( 'curPageURL' ) ){
    /**
     * Get current page URL / get_permalink() can be used too
     *
     * @since 1.0
     *
     * @param   bool    $queryPram  $queryPram if you want to get url with query string, false otherwise
...
if( ! function_exists( 'get_terms_from_taxonomies' ) ){
    /**
     ...
     * @param array $args Query arguments (check $defaults variable)
     *
     * @link    https://codex.wordpress.org/Function_Reference/get_terms
     *
     * @return ARRAY|STRING|WP_Error Array of term object
     */
    function get_terms_from_taxonomies( $taxonomies, $args = array() ){
...
if( ! function_exists( 'insert_post' ) ){
    /**
     * Insert a post to any post type
     *
     * @since 1.0
     * @access public
     *
     * @param array $args Arguments to insert a post
     *
     * @link https://codex.wordpress.org/Function_Reference/wp_insert_post
     *
     * @return INT|WP_Error ID of the inserted post or WP_Error
     */
...
if( ! function_exists( 'get_all_posts' ) ){
    /**
     * Get all posts from any post type
     *
     * @since 1.0
     * @access public
     *
     * @param array $args Arguments to fetch posts
     *
     * @link https://codex.wordpress.org/Class_Reference/WP_Query
     *
     * @return OBJECT Object of posts
     */
    function get_all_posts( $args ){
        $defaults = array(
            'post_status'           => 'publish',
            'post_type'             => 'post',
...
if( ! function_exists( 'checked_select' ) ){
    /**
     * Check two values and find out if they are equal or not for select dropdown, radio, checkbox etc
     *
     * @since 1.0
     * @access public
     *
     * @param ANY $current_value The value changes in the loop
     * @param ANY $matched_value The value should be matched
     * @param STRING $type Select drodown, or radio or checkbox
     *
     * @return STRING Checked if equal, otherwise null
     */
    function checked_select( $current_value, $matched_value ){
        if( $current_value == $matched_value ){
            return 'selected';
        }
    }
}
...
function is_user_going( $user_id = 0, $event_id = 0 ) {
	...
	$event = new EP_Event( $event_id );
	return $event->is_user_going( $user_id );
